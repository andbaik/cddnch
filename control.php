...
if ($error_print == true) {
    echo "Имя ДНЧ - $row_user->dnch_name  LEVEL = $level";
}

//если не ДНЧ то авторизация
if ($level !== 4) {
    header('Location:' . $site);
}
...
switch ($week_set) {
    ...
    default:
        $error = 'Дни недели не сформированы!';
        break;
}
//Проверить после заполнения//
//запрос, текущая неделя заполнена
$query_week = $pdo->query("SELECT * FROM `controls` WHERE `id_dnch` = $row_user->id_dnch AND weekNumber = $weekNumber");
$row_week = $query_week->fetch(PDO::FETCH_OBJ);

print_r($query_week);

if (!empty($row_week)) {
    header('Location:' . $site . 'control_dnch.php');
}
//Проверить после заполнения!!!!!//
...
<body class="wrapper">
    ...
    <main class="main">
        <div class="container">
            ...
            <div class="heading">
                <h1 class="text-center">Управление процессом деятельности Инструктора по безопасности <?= $row_user->dnch_name ?></h1>
            </div>
            <form action="#" method="POST" id="add_events" enctype="multipart/form-data">
                <div class="workspace">
                    <div class="workspace_header d-flex justify-content-lg-around">
                        <div class="workspace_header_choice">
                            <!--
                        <div class="header_choice d-flex flex-md-column py-3">
                            <div class="choice_week my-2">
                                <span>Выберите неделю</span>
                                <input type="week" name="weekNumber" id="weekNumber" value="<?= $weekNumber ?>">
                            </div>
                            <div class="button_ok my-2">
                    ...
                    <div class="workspace_week d-flex justify-content-md-between">
                        ...
                        <input type="text" value="<?= $row_user->id_dnch ?>" name="id_dnch" id="id_dnch" hidden>
                        <input type="text" value="<?= $id_user ?>" name="id_user" id="id_user" hidden>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data == 1) {
                        $("#sub").text("Выполнено");
                        $("#error-block").hide();
                        window.location.replace("http://cddnch.loc/control_dnch.php");
