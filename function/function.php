...
class DateClass
{
    public static function get_day($date_now){
        $date = $date_now;
        $day = date('d', strtotime($date));
        return $day;
    }
    public static function get_month($date_now){
        $date = $date_now;
        $month = date('m', strtotime($date));
        return $month;
    }
    public static function get_year($date_now) {
        $date = $date_now;
...
function getWeekNumber($mondayDateStr) {
    // $mondayDateStr должен быть в формате 'YYYY-MM-DD'
    $date = new DateTime($mondayDateStr);
    // format('W') возвращает номер недели ISO
